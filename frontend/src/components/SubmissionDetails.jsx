...
export function SubmissionDetails({ document, status, hideTitle = false, children }) {
  return <aside className="department-submission-review__details">
    ...
    <SubmissionDetailSection title="Agreement Details"><SubmissionDetail label="Document Type" value={document.document_type} />{!hideTitle && <SubmissionDetail label="Title of Agreement" value={document.title} />}<SubmissionDetail label="Partner Organization" value={document.partner_institution} /><SubmissionDetail label="Partner Contact Email" value={document.partner_email} /></SubmissionDetailSection>
