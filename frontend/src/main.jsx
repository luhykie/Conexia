...
class AppErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, errorInfo) {
    reportClientError("React render error:", error, errorInfo);
  }
...
function App() {
  ...
  React.useEffect(() => {
    ...
    async function restoreSession() {
      const restoredAccount = await getAuthenticatedAccount();
    ...
    const unsubscribe = subscribeToAuthChanges((nextAccount) => {
      if (active) {
  ...
  return (
    <Routes>
      ...
      <Route
        ...
        element={
          account ? (
            <WorkspaceRoute account={account} onLogout={handleLogout} />
          ) : (
...
function LoginRoute({ onLogin }) {
  ...
  function handleSuccessfulLogin(nextAccount) {
    onLogin(nextAccount);
    navigate(`/app/${getDefaultPage(nextAccount.roleKey)}`, {
      replace: true,
    });
  }
...
function WorkspaceRoute({ account, onLogout }) {
  ...
  function navigateToPage(nextPage) {
    if (!canAccessPage(account.roleKey, nextPage)) {
      navigate(`/app/${getDefaultPage(account.roleKey)}`, {
        replace: true,
      });
      return;
    }
  ...
  return (
    <AppLayout account={account} onLogout={handleLogout}>
  ...
  return (
    <main className="welcome-page">
      ...
      <section className="welcome-hero" id="product">
        <div className="hero-copy">
          <span className="trusted-pill">
            <ShieldCheck size={15} /> Trusted by 500+ universities
          </span>
          <h1>
          ...
          <p>
            The definitive OS for global academic document management. Automate
            workflows with impenetrable security and real-time compliance.
          </p>
      ...
      <section className="welcome-features" id="solutions">
        ...
        <p>
          A unified platform for secure academic document management and global
          partnership scaling.
        </p>
      ...
      <section className="security-band" id="security">
        <div>
          ...
          <p>
            Protecting sensitive data with the world&apos;s most rigorous
            security certifications.
          </p>
        </div>
        <div className="cert-row">
          <span>
            <ShieldCheck /> SOC2 TYPE II
          </span>
          <span>
            <Shield /> ISO 27001
          </span>
          <span>
            <ShieldCheck /> HIPAA READY
          </span>
          <span>
            <Shield /> FERPA COMPLIANT
          </span>
        </div>
      </section>
      ...
      <section className="welcome-cta">
        ...
        <p>
          Join the world&apos;s leading universities in streamlining
          documentation and scaling global academic cooperation effortlessly.
        </p>
        <div>
      ...
      <footer className="welcome-footer">
        <div>
          ...
          <p>
            Secure and compliant document management for the global higher
            education ecosystem.
          </p>
        ...
        <small>
          © 2024 CONEXIA University Document Systems. All rights reserved.
  ...
  return (
    <main className="auth-screen">
      <form className="auth-card" onSubmit={handleSubmit}>
        <img className="auth-logo" src={conexiaLogo} alt="CONEXIA" />
        <h1>CONEXIA</h1>
