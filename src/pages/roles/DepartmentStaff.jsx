  ...
  React.useEffect(() => {
    ...
    async function loadSubmissions() {
      const { data, error } = await supabase
        .from("submissions")
        .select("id, partner_institution_name, agreement_type, status, created_at, legal_comments")
  ...
  async function handleResubmit() {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8000/api/submissions/${selectedSubmission.id}/status`, {
        ...
        body: JSON.stringify({
          status: "resubmitted",
          notes: "Document resubmitted after corrections",
        }),
      });

      if (response.ok) {
        setMessage("Document resubmitted for legal review.");
        setSelectedSubmission(null);
        loadSubmissions();
      } else {
        setMessage("Failed to resubmit document. Please try again.");
      }
    } catch (error) {
      setMessage("Failed to resubmit document. Please try again.");
    }
      ...
      <div>
        ...
        <Panel title="Submission Records">
          {loading ? (
            ...
            <DataTable headers={["Tracking #", "Partner", "Type", "Status", "Action"]} rows={rows} />
          )}
        </Panel>
      </div>
      <aside className="detail-drawer">
        ...
        {selectedSubmission ? (
          <>
            ...
            <p><strong>Type:</strong> {selectedSubmission.agreement_type}</p>
            <p><strong>Status:</strong> <b>{selectedSubmission.status}</b></p>
            {selectedSubmission.legal_comments && (
              <div style={{ marginTop: "16px", padding: "12px", backgroundColor: "#fff3cd", borderRadius: "4px" }}>
                <strong>Legal Comments:</strong>
                <p>{selectedSubmission.legal_comments}</p>
              </div>
            )}
            {selectedSubmission.status === "corrections_needed" && (
              <button 
